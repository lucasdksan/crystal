...
function computeProductScores(productStats) {
...
function computePortfolioScores(productScores, cancelRate, dependencyScore) {
    const totalEffectiveQty = productScores.reduce((acc, p) => acc + p.effectiveQty, 0);
    ...
    const avgBundleScore =
        productScores.length > 0
            ? productScores.reduce((acc, p) => acc + p.bundleScore, 0) / productScores.length
            : 0;
...
function buildKitSuggestions(productScores, risks) {
    const kits = [];
    const sortedByVolume = [...productScores].sort((a, b) => b.effectiveQty - a.effectiveQty);
    const champion = sortedByVolume[0];
    const runnerUp = sortedByVolume[1];

    if (champion && runnerUp && champion.bundleScore >= 0.4 && runnerUp.bundleScore >= 0.3) {
        kits.push({
...
function buildRiskMitigationStrategy(portfolioScores, risks, cancelRate) {
    ...
    return {
        ...
        confidenceScore: clamp(portfolioScores.portfolioRiskScore * 0.7 + portfolioScores.cancelRateNorm * 0.3),
...
function buildDiversificationStrategy(portfolioScores, productScores, championProduct) {
        ...
        actions: [
            {
                ...
                description: `Impulsionar itens fora do top 1: ${topProducts.slice(1).map((p) => p.label).join(", ") || "N/A"}`,
            },
            {
...
function buildKitOpportunityStrategy(portfolioScores, productScores, risks) {
    const kits = buildKitSuggestions(productScores, risks);
    const stagnantCount = risks.filter((r) => r.riskType === RISK_TYPE_LABEL.STAGNANT_STOCK).length;
...
function buildConversionRecoveryStrategy(portfolioScores, productScores, cancelRate) {
...
function buildExpansionStrategy(portfolioScores, productScores, championProduct) {
    ...
    return {
        ...
        actions: [
            ...
            },
            {
                ...
                description: "Operação estável — testar upsell e cross-sell sem pressão de risco",
            },
        ],
    };
}

function buildStabilityMaintenanceStrategy(portfolioScores, championProduct, bottleneckProduct) {
    return {
        ...
        actions: [
            {
                ...
                description: "Acompanhar cancelamento, entrega e concentração semanalmente",
            },
            {
...
function buildStrategicRecommendations({
    ...
    if (portfolioScores.portfolioRiskScore > 0.6 || cancelRate > 30) {
        strategies.push(buildRiskMitigationStrategy(portfolioScores, risks, cancelRate));
    }

    if (portfolioScores.dependencyScore > DEPENDENCY_THRESHOLD) {
        strategies.push(
            buildDiversificationStrategy(portfolioScores, productScores, championProduct)
        );
    }

    if (hasHighBundlePotential && (stagnantCount > 0 || portfolioScores.avgBundleScore >= 0.5)) {
        const kitStrategy = buildKitOpportunityStrategy(portfolioScores, productScores, risks);
    ...
    if (cancelRate > 15 && portfolioScores.cancelRateNorm > 0.15) {
        strategies.push(
            buildConversionRecoveryStrategy(portfolioScores, productScores, cancelRate)
        );
    }
...
function buildDiagnostics(rawList) {
    ...
    const dependencyScore =
        totalEffectiveQty > 0 ? (sortedByVolume[0]?.effectiveQty ?? 0) / totalEffectiveQty : 0;
    ...
    const portfolioScores = computePortfolioScores(productScores, cancelRate, dependencyScore);
    ...
    return { diagnosis, risks, strategies, productStats, productScores, portfolioScores };
}
